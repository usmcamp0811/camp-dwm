...
static const char *colors[][3]      = {
	...
	[SchemeSel]         = { col_gray4, col_alt_black,  col_yellow  },
	[SchemeStatus]      = { col_gray3, col_red,  "#000000"  }, // Statusbar right {text,background,not used but cannot be empty}
...
/* static const char *dmenucmd[] = SHCMD("rofi -show drun -font \"IBM Plex Mono 12\" -run-shell-command '{terminal} -e \" {cmd}; read -n 1 -s\"'"); */
static const char *dmenucmd[] = { "dmenu_run", "-m", dmenumon, "-fn", dmenufont, "-nb", col_alt_black, "-nf", col_gray3, "-sb", col_cyan, "-sf", col_gray4, NULL };
...
static Key keys[] = {
	...
	/* { MODKEY|ShiftMask,                   XK_i,		setlayout,	{.v = &layouts[7]} }, /1* bstackhoriz *1/ */
	TAGKEYS(                        XK_1, 0)
	TAGKEYS(                        XK_2, 1)
	TAGKEYS(                        XK_3, 2)
	TAGKEYS(                        XK_4, 3)
	...
	{ 0,                                  XF86XK_MonBrightnessUp,	spawn,		SHCMD("~/.local/bin/backlight +") },
	{ 0,                                  XF86XK_MonBrightnessDown,	spawn,		SHCMD("~/.local/bin/backlight -") },
	{ Mod4Mask,                           XK_l,           spawn,		SHCMD("i3lock-fancy") },
};
